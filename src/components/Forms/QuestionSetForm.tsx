  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (questionSetData?.id) {
        await QuestionSetServices.updateQuestionSet(questionSetData.id, payload);
      } else {
      ...
      setError(err.message || "An error occurred while saving the question set");
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {success && (
        <div className="rounded-md bg-green-50 p-4 text-sm text-green-700">
          Question set {questionSetData?.id ? "updated" : "created"} successfully!
        </div>
      )}
